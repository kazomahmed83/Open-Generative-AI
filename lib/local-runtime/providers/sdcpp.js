...
async function generate({ model, params }, onProgress = () => {}, { signal } = {}) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    const onAbort = () => { try { proc.kill('SIGTERM'); } catch {} cleanupTmp(); reject(new Error('Generation cancelled')); };
    if (signal) {
      if (signal.aborted) { try { proc.kill('SIGTERM'); } catch {} cleanupTmp(); return reject(new Error('Generation cancelled')); }
...
module.exports = { resolveSeed, arToDimensions, buildSdCppArgs, generate };
