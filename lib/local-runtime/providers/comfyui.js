...
function buildFlux2Workflow({ prompt, negativePrompt = '', width = 1024, height = 1024, steps = 20, guidance = 3.5, seed = 0, unetName, clipName, vaeName, samplerName = 'euler', scheduler = 'simple' }) {
  return {
    ...
    // FLUX carries guidance in the conditioning (not classifier-free cfg), so the positive cond goes
    // through FluxGuidance and KSampler runs at cfg 1. The latent is 16-channel (EmptySD3LatentImage);
    // the SD-style 4-channel EmptyLatentImage causes a tensor-shape mismatch in the FLUX transformer.
    '13': { class_type: 'FluxGuidance', inputs: { conditioning: ['6', 0], guidance } },
    ...
    '3': { class_type: 'KSampler', inputs: { seed, steps, cfg: 1, sampler_name: samplerName, scheduler, denoise: 1, model: ['10', 0], positive: ['13', 0], negative: ['7', 0], latent_image: ['5', 0] } },
...
async function generate({ model, params, baseUrl = DEFAULT_BASE }, onProgress = () => {}, { signal } = {}) {
  ...
  if (recipe && recipe.workflow === 'flux2') {
    ...
    workflow = buildFlux2Workflow({
      ...
      width, height, steps: params.steps || 20, guidance: params.guidance_scale || 3.5, seed,
